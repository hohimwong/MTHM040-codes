include("2D_JW_alpha2omega_space_time.jl")
# script to run the code for the 2D case with space and time dependencies implemented; the α^2ω case investigated in the report is done using this code, by setting ϵ=-1
# do note that "y" here actually corresponds to "z" in the actual model

param = [-100000,1,1,1]      # first element corresponds to D
x_bdy = [0,pi]
y_bdy = [0,0.5]
δt = 0.001
...
#for l in 1:m_pts
#    A0[l,:] = [(0.3*sin(k) + 0.3*cos(2*k)) for k in LinRange(x_bdy[1],x_bdy[2],n_pts)]
#    B0[l,:] = [(0.3*sin(k) + 0.7*cos(k)) for k in LinRange(x_bdy[1],x_bdy[2],n_pts)]
#end
#for l in 1:m_pts
...
results = TwoDJW_alp2ome_ST(param, δx, δy, δt, x_bdy, y_bdy, i, A0, B0, x_bdy_condi, y_bdy_condi, κ=1, τ=0, λ=1, ux=1, update=0,ϵ=0,tol=10e-3)
# can set τ=1,ϵ=-1 for the results in the α^2ω regime
A_results = results[1]
B_results = results[2]
transient_steps = 0
...
Colorbar(f[2,1], colormap =:hsv, limits = (findmin(B_results_matrix_y)[1], findmax(B_results_matrix_y)[1]), label = "value of B",vertical=false)
...
contour!(ax2, A_results_matrix, colormap =:hsv, levels=30)
Colorbar(f2[1,2], colormap=:hsv, limits=(findmin(A_results_matrix)[1], findmax(A_results_matrix)[1]), label="value of A")
