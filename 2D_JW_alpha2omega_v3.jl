...
function u(invar; amp=0.0, phase=0.0, freq = 1.7, l=i, m=m_pts, n=n_pts)       # function taking in ω0 and spits out u with time/space dependence (3d array)
    
    output = zeros(l+1,m,n)
    
    for lloop in 1:l+1 # time direction
        for mloop in 1:m # z direction
            for nloop in 1:n # x direction
...
# same as the testing code, the A and B's in the function name refers to the unknown its for, and the x and y's are refering to which direction its doing a sweep in
...
# ψ is now a switch for the ∂u/∂x * ∂A/∂z term
...
function TwoDJW_alp2ome_v3(param, δx, δy, δt, x_bdy, y_bdy, i, A0, B0, x_bdy_condi, y_bdy_condi; κ=1,τ=1,λ=1,ux=1,update=1,ϵ=0,tol=10e-3)

    c1 = (2*δx^2)/(δt)
    c2 = (2*δy^2)/(δt)

    α0 = copy(param[1]) # α0 effectively corresponds to D
    ω0 = copy(param[2])
    η = copy(param[3])
    L = copy(param[4])
    ...
    for j in 2:i+1 # timestep loop
        ...
        u_fn = copy(u_container[j-1,:,:])     # taking the values of u and α at a given time
        α_fn = copy(α_container[j-1,:,:])

        u_fn_new = copy(u_container[j,:,:])
        α_fn_new = copy(α_container[j,:,:])

        while >(err_A,tol) || >(err_B,tol) # loop for adi, now we iterate as long as either error is above the tolerance
        ...
        if update == 1
            ...
            if rem(j,5) == 0 # make a snapshot if j is divisible by the second argument
                fig = Figure()
