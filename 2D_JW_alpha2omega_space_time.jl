...
# code for the 2D case with space and time dependencies implemented
# do note that the "y" here corresponds to "z" in the actual model, and the code no longer has a switch for source terms

function u(invar; amp=0.1, phase=0.0, freq = 0.5, l=i, m=m_pts, n=n_pts, x_bdy=x_bdy, y_bdy=y_bdy)       # function taking in ω0 and spits out u with time/space dependence (3d array)
    
    output = zeros(l+1,m,n)

    spa_dep = TwoD_diff_rot(m,n,x_bdy,y_bdy,1)
    for lloop in 1:l+1 # time loop
        for mloop in 1:m # z loop
            for nloop in 1:n # x loop
                
                output[lloop,mloop,nloop] = sin((nloop-1)*δx)*(amp*cos(2*pi*freq*(lloop)*δt) + amp*sin(2*pi*freq*(lloop)*δt)) + spa_dep[mloop,nloop]
...
function α(invar; amp=0.1, phase=0.0, freq = 0.5, l=i, m=m_pts, n=n_pts)        
    ...
    for lloop in 1:l+1
        for mloop in 1:m 
            for nloop in 1:n 
                ...
                output[lloop,mloop,nloop] = invar*( amp*sin(2*pi*freq*(lloop)*δt) ) +
                ( (cos((nloop-1)*δx)) * (1+tanh(4*((mloop-1)*δy-0.4))) )
...
function TwoDJW_alp2ome_ST(param, δx, δy, δt, x_bdy, y_bdy, i, A0, B0, x_bdy_condi, y_bdy_condi; κ=1,τ=1,λ=1,ux=1,update=1,ϵ=0,tol=10e-2)

    c1 = (2*δx^2)/(δt)
    c2 = (2*δy^2)/(δt)

    α0 = copy(param[1]) # effectively D
    ω0 = copy(param[2])
    η = copy(param[3])
    L = copy(param[4])
    ...
    A_soln_list[1] = A0
    B_soln_list[1] = B0
    D = (α0*ω0*(L^3))/(η^2) # the same as α0 with our non-dimensionalisation
    @show(D)
    ...
    for j in 2:i+1 # timestep loop
        ...
        u_fn = u_container[j-1,:,:]
        α_fn = α_container[j-1,:,:]
        
        u_fn_new = u_container[j,:,:]
        α_fn_new = α_container[j,:,:]

        while >(err_A,tol) || >(err_B,tol) # loop for ADI, only stops when both error are below the tolerance
